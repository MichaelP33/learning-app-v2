...
export function calculateProgress(articles: Article[]): Progress {
  ...
  const completed = articles.filter(
    (a) => a.learningStatus === "Completed"
  ).length;
  const inProgress = articles.filter(
    (a) => a.learningStatus === "In progress"
  ).length;
  const reviewing = articles.filter(
    (a) => a.learningStatus === "Reviewing"
  ).length;
  const notStarted = articles.filter(
    (a) => a.learningStatus === "Not started"
  ).length;
...
export function calculateSelfAssessmentScore(
  points: number,
  assessment: "nailed-it" | "mostly-good" | "not-quite"
): number {
  switch (assessment) {
    case "nailed-it":
      return points;
...
export function updateHighScores(
  ...
  const highScores =
    getQuizHighScores(articleId) || initializeHighScores(articleId);
  ...
  currentAnswers.forEach((answer) => {
    const currentQuestionScore =
      highScores.questionScores[answer.questionId] || 0;
...
export function hasEverAnsweredQuestionCorrectly(
  articleId: string,
  questionId: number
): boolean {
...
export function getQuestionMasteryCount(
  articleId: string,
  ...
  const mastered = Object.values(highScores.questionScores).filter(
    (score) => score > 0
  ).length;
  return { mastered, total: totalQuestions };
}

// New Proficiency Calculation Functions
export function calculateArticleProficiency(articleId: string): number {
  const article = getArticleById(articleId);
  if (!article?.quiz) return 0;

  const highScores = getQuizHighScores(articleId);
  return highScores?.bestOverallPercentage || 0;
}

export function calculateTopicProficiency(topicId: string): number {
  const topic = getTopicById(topicId);
  if (!topic) return 0;

  const totalProficiency = topic.articles.reduce((sum, article) => {
    return sum + calculateArticleProficiency(article.id);
  }, 0);

  return topic.articles.length > 0
    ? Math.round(totalProficiency / topic.articles.length)
    : 0;
}

export function calculateCategoryProficiency(categoryId: string): number {
  const category = getCategoryById(categoryId);
  if (!category) return 0;

  const allArticles = category.topics.flatMap((topic) => topic.articles);
  const totalProficiency = allArticles.reduce((sum, article) => {
    return sum + calculateArticleProficiency(article.id);
  }, 0);

  return allArticles.length > 0
    ? Math.round(totalProficiency / allArticles.length)
    : 0;
}

export function calculateOverallProficiency(): number {
  const categories = getAllCategories();
  const allArticles = categories.flatMap((category) =>
    category.topics.flatMap((topic) => topic.articles)
  );
